...
struct TestHelpers {
    ...
    func createUser(app: Application, email: String) async throws -> User {
        ...
        let user = try await User.query(on: app.db).filter(
    ...
    func createCircle(app: Application, name: String) async throws -> Circle {
        let body = CreateCircleRequest(
            ...
            pictureUrl:
                "https://cdn.example.com/circles/\(UUID().uuidString).png"
        )
        try await app.test(
            .POST,
            circlesRoute,
        ...
        let circle = try await Circle.query(on: app.db).filter(
    ...
    func createPost(
        ...
        let post = try await Post.query(on: app.db).filter(
            \.$content == InputSanitizer.sanitizeTextBlock(content)
        ).first()
        return try #require(post)
    }

    func createEvent(app: Application, title: String) async throws
        -> CalendarEvent
    {
        ...
        try await app.test(
            .POST,
            "\(circlesRoute)/\(body.circleID.uuidString)/\(eventsSegment)/user/\(body.hostID.uuidString)",
            beforeRequest: { req in
                try req.content.encode(body)
            },
            afterResponse: { res in
        ...
        let evt = try await CalendarEvent.query(on: app.db).filter(
    ...
    func createComment(
        ...
        authorID: UUID,
        content: String
    ) async throws -> SonderBackend.Comment {
        ...
        try await app.test(
            .POST,
            "\(circlesRoute)/\(circleID.uuidString)/\(postsSegment)/\(postID.uuidString)/\(commentsSegment)/user/\(authorID.uuidString)",
            beforeRequest: { req in
                try req.content.encode(body)
            },
            afterResponse: { res in
        ...
        let comment = try await SonderBackend.Comment.query(on: app.db).filter(
            \.$content == InputSanitizer.sanitizeTextBlock(content)
        ).first()
